...
immonet_path_raw = "Data/RawData/RawData_immonet/"
immonet_path_merged = "Data/MergedData/MergedData_immonet/"
immonet_path_clean = "Data/CleanData/"


directories_raw = os.listdir(immonet_path_raw)
directories_clean = os.listdir(immonet_path_clean)
...
def Table_immonet_crate():
    ...
    try:
        table_craation_line = "CREATE TABLE immonet ("
        counter = 0
...
def Table_immonet_copying():
    # example how to import csv and work in json

    files: list = glob.glob(immonet_path_raw + "*.csv")
    ...
    with open(immonet_path_merged + "MergedData_immonet.json", "w") as f:
        json.dump(data_json, f, indent=4)


def Table_immonet_cleaning():
    # open the merged file and convert it to a dictionary
    new_merged_dict = {}
    clear_dict = {}
    with open(immonet_path_merged + "MergedData_immonet.json", "r") as f:
        data = json.load(f)
    ...
    f = open(immonet_path_clean + "CleanData_immowelt.json", "w")
    json.dump(clear_dict, f, indent=4)
    logger.info('CleanData_immonet was created')

    # Inserting a merged dict into a new file
    f = open(immonet_path_merged + "NewMergedData_immowelt.json", "w")
    json.dump(new_merged_dict, f, indent=4)
    logger.info('MergedData_immonet was created')


def Table_immonet_fill():
    with open(immonet_path_clean + "CleanData_immowelt.json", "r") as f:
        data = json.load(f)

    # creating the first part of the query to create the table
    try:
        table_fill_line_TableInfo = "INSERT INTO immonet ("
        counter = 0
