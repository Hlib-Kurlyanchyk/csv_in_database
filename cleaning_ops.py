...
def old_str_to_float(data_price: str) -> float | None:
    ...
    elif not isinstance(data_price, str):
        return None
...
def str_to_float(data_price: str) -> float | None:
    if isinstance(data_price, int):
        return float(data_price)
        ...
        return None
        #raise TypeError("The data has the wrong type")

    elif re.search(r"\d", data_price) is None:
        return None
        #raise TypeError('The data has no numbers')

    else:
        index_last_num = re.match('.+([0-9])[^0-9]*$', data_price)
        data_price = data_price[:index_last_num.start(1) + 1]

        index_first_num = re.search(r"\d", data_price)
        data_price = data_price[index_first_num.start():]
...
def test_price_to_float():
    assert old_str_to_float("1.230 ") == 1230.0
